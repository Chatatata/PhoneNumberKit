...
extension PhoneNumberFormatter {
    ...
    override open func isPartialStringValid(_ partialStringPtr: AutoreleasingUnsafeMutablePointer<NSString>,
        ...
        let action = self.action(for: origString as NSString, range: origSelRange, proposedString: partialStringPtr.pointee, proposedRange: proposedSelRangePtr.pointee)
        if action == .delete && (origString as NSString).isWhiteSpace(in: origSelRange) {
        ...
        if action == .insert || action == .replace {
            ...
            let length = partialStringPtr.pointee.length - (origString as NSString).length + origSelRange.length
